        ...
        function applyCylinderLayout() {{
            ...
            if (typeof THREE !== 'undefined' && ringsEnabled) {{
                ...
                stageDefinitions.forEach((stage, index) => {{
                    ...
                    // Create flat ring disc with small inner radius (10% of outer) for transparency
                    const outerRadius = radius + 50;
                    const innerRadius = outerRadius * 0.1;  // Small inner hole
                    const ringGeom = new THREE.RingGeometry(innerRadius, outerRadius, 64);
                    const ringMat = new THREE.MeshBasicMaterial({{
                        color: ringColor,
                        transparent: true,
                        opacity: currentTheme === 'light' ? 0.08 : 0.12,
                    ...
                    // Add subtle edge ring for definition
                    const edgeGeom = new THREE.RingGeometry(outerRadius - 2, outerRadius, 64);
